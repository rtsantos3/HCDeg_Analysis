...
kegg_strat_relcounts <- strat_kegg %>% transmute(description = as.factor(description), sample, taxon = taxa_vsearch[taxon, 'Genus'], taxon_rel_function_abun) %>% group_by(sample, description,taxon) %>% summarise(across(taxon_rel_function_abun, sum)) %>% filter(description %in% deg_df$pathway_desc) #Summarise based on taxon_rel 
...
fam_cols  <- colorRampPalette(brewer.pal(12, "Set3"))(18)


pdf(file = "./out/Per-taxon-heatmap.pdf",   
    width = 10, 
    height = 7) 
...
class_counts_sum <- class_counts_sum %>% transmute()
