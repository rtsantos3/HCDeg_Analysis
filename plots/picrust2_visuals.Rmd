...
fam_cols  <- colorRampPalette(brewer.pal(12, "Paired"))(20)
...
```{r}
fam_cols2  <- wes_palette("Darjeeling2", 12, type = "continuous")
...
geom_bar(position="stack", stat="identity")  +   
scale_fill_manual(values=fam_cols2, drop=T) +
labs(y= "Function relative abundance (Per taxon)", x="HC Degradation Pathways (Level 3 KEGG)")+ coord_flip() + facet_wrap(~sample, nrow = 1)
...
plot8 <- ggplot(kegg_strat_relcounts_genus, aes(x=description, y=taxon_rel_function_abun, fill = taxon)) +
geom_bar(position="stack", stat="identity")  +   
scale_fill_manual(values=fam_cols, drop=T) +
labs(y= "Function relative abundance (Per taxon)", x="HC Degradation Pathways (Level 3 KEGG)")+ coord_flip() + facet_wrap(~sample, nrow = 1)
