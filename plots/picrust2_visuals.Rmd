...
kegg_strat_relcounts <- strat_kegg %>% transmute(description, sample, taxon = taxa_vsearch[taxon, 'Family'], taxon_rel_function_abun) %>% group_by(sample, description,taxon) %>% summarise(across(taxon_rel_function_abun, sum)) %>% filter(description %in% deg_df$pathway_desc) #Summarise based on taxon_rel

write.table(kegg_strat_relcounts, file = './intermediate_files/selected_relative_abundance_KEGG_pathways.tsv', sep ='\t', col.names = T)
