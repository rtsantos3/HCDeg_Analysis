...
#This code block is to load the required files to plot NSTI vs relative abundance
abs_count_otu_qza <-  '../dada2/table.qza'
...
pdf(file = "./out/16S-relative-otu-plot-Class.pdf",   
    width = 6, 
    height = 15) 



phy_fam <- phyloseq_qza %>% aggregate_taxa(level = "Class")  %>% microbiome::transform(transform = 'compositional')
rel_abund_plots <- plot_composition(x = phy_fam,otu.sort = 'abundance') + 
  scale_fill_manual(values = fam_cols)

plot(rel_abund_plots)
```
